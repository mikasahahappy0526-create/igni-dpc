...
## v1.0.58（アライブ 0.1.92・署名鍵の変更）

- **AliveInstaller**: 公開アライブ **0.1.92** を GitHub Releases の `alive.apk` に固定。`latest/download` を一次、検証済みの `0.1.92` タグ URL をフォールバックに使用
- 両 URL のダウンロード後に SHA-256 `a7a80b9ffa9cde893853bc697c21e585b8e9b62452fa0df8782ec38154eb6894`（1,471,139 bytes）を検証し、不一致はハード失敗（日本語ステータス）
- **署名証明書を更新**: 0.1.92 は署名鍵をローテーションした。ピンの証明書 SHA-256 は `18faf84a7543ea4dbcfaeba1cd2f94a2d5410e8912b890a1fe39d37e86bea4b8`。旧証明書 `106691866d324942d8ad8bbe5722b59c2aceb35b0532692008a59248467f92c1` は以降使わない
- **既存のアライブ**: 古い署名のアライブが入っている端末は上書き更新できない。一度アンインストールしてから 0.1.92 を入れ直す。Igni は署名違いを「要アンインストール」と表示し、サイレントアンインストールはしない
- 未インストールの端末は 0.1.92 を導入する。ピン以上かつ新しい証明書と一致するときだけ開く
